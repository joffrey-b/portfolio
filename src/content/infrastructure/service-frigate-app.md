...
Frigate is the open-source NVR software recording the homelab's IP cameras, running as its own Docker Compose stack on a dedicated Debian VM with an encrypted drive so footage stays safe. It replaced ZoneMinder as the active system; this deployment is recording-only, with object detection and Home Assistant integration switched off since the host has no GPU.

Camera feeds can be viewed live, and up to 10 days of past footage is kept before it rolls off.
